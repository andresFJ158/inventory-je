...
class FilesController{
	...
	public function ajaxUploadFiles(){
		...
		try {
			...
			if(isset($folder->status) && $folder->status == 200 && !empty($folder->results)){
				...
				if($this->folder == 1){
					...
					if(move_uploaded_file($this->file["tmp_name"], $path)){
						...
						if($uploadData->status == 200){
							...
							echo json_encode(array("status" => 500, "error" => "Error al guardar en DB: " . json_encode($uploadData)));
							return;
						}

					} else {
						echo json_encode(array("status" => 500, "error" => "Error al mover el archivo a $path"));
						return;
					}

				} else {
					echo json_encode(array("status" => 500, "error" => "Folder no soportado actualmente"));
					return;
				}
			
			} else {
				echo json_encode(array("status" => 404, "error" => "No se encontró información del folder o la API no respondió"));
				return;
			}
		} catch (Exception $e) {
			echo json_encode(array("status" => 500, "error" => "Exception: " . $e->getMessage()));
		}
		...
		if($files->status == 200){
			...
			foreach ($files as $key => $value) {
				...
				if($countFiles == count($files)){
					...
					if(isset($folders->status) && $folders->status == 200){
	...
	public function deleteFile(){
		...
		if($this->idFolderDelete == 1){
			...
			=============================================*/
			$fileName = basename($getFile->link_file);
			$localPath = "../views/assets/files/".$fileName;
		...
		if(isset($updateFolder->status) && $updateFolder->status == 200 && isset($deleteFile->status) && $deleteFile->status == 200){

			echo $deleteFile->status;
		} else {
			echo json_encode(array("updateFolder" => $updateFolder, "deleteFile" => $deleteFile));
		}
