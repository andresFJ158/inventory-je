...
ini_set('error_log', '/tmp/pos_ajax_errors.log');
...
// Global exception handler — registra el detalle pero no lo expone en producción
set_exception_handler(function($e) use ($POS_DEBUG) {
	error_log("EXCEPTION in pos.ajax.php: " . $e->getMessage() . " at " . $e->getFile() . ":" . $e->getLine());
	// Si una transacción quedó abierta por la excepción, revertirla
	try {
...
class LocalConnection {
	...
	static public function connect(){
		// Singleton por request: reutiliza una sola conexión PDO.
		// Evita reconexiones que romperían transacciones (cada PDO tiene su propia
		// transacción) y reduce la presión de conexiones sobre MariaDB.
		...
		// EMULATE_PREPARES se deja en true (default): varios queries del sistema
		// reutilizan el mismo placeholder nombrado (ej. ":qty" dos veces), lo cual
		// solo es válido con emulación activada. Cambiarlo a false los rompería.
	...
	static private function ensureRuntimeSchema($link){
		if(self::$schemaReady){
			return;
		}
...
if (isset($_POST["transferStockBetweenOffices"])) {
	...
	try {
		...
		if ($avail < (double)$qty) {
			echo "error: Stock insuficiente en el almacén de origen.";
			$db->rollBack();
			exit;
		}
		...
		// Subtract from source office stock
		$stmtDecrease = $db->prepare("
			UPDATE product_inventory
			SET stock_inventory = GREATEST(0, stock_inventory - :qty)
			WHERE id_product_inventory = :prod AND id_office_inventory = :office AND status_inventory = 1
...
// Handler getLabMaterials consolidado arriba (eliminado duplicado)
...
if (isset($_POST['confirmOrderPayment'])) {
    ...
    if (!$id) { pos_fail(400, 'Orden inválida'); }

    // El archivo se guarda fuera de la transacción (I/O de filesystem).
    // Si falla la validación, no abortamos el pago: registramos sin archivo y avisamos.
...
// Guarda un archivo de comprobante en views/assets/files y devuelve la ruta pública (o null)
...
// Adjuntar/reemplazar comprobante después de la venta (desde Ventas/Facturación)
if (isset($_POST['uploadSalePayment'])) {
    $db = LocalConnection::connect();
    $idOrder = intval($_POST['id_order'] ?? 0);
    if (!$idOrder) { echo json_encode(['status' => 400, 'message' => 'Orden inválida']); exit; }
    try {
        $filePath = posStoreSalePaymentFile($_FILES['proof'] ?? null);
    } catch (Exception $e) {
...
if (isset($_POST['getComboItems'])) {
    $db = LocalConnection::connect();
    $stmt = $db->prepare("SELECT ci.*, p.title_product, p.sku_product, p.unit_product FROM combo_items ci JOIN products p ON ci.id_product=p.id_product WHERE ci.id_combo=:id");
...
if (isset($_POST['saveComboItem'])) {
    ...
    if (isset($_POST['id_combo_item']) && $_POST['id_combo_item']) {
        $stmt = $db->prepare("UPDATE combo_items SET id_product=:p, qty_combo_item=:q WHERE id_combo_item=:id");
        $stmt->execute([':p' => intval($_POST['id_product']), ':q' => floatval($_POST['qty']), ':id' => intval($_POST['id_combo_item'])]);
    } else {
        $stmt = $db->prepare("INSERT INTO combo_items (id_combo, id_product, qty_combo_item) VALUES (:c, :p, :q)");
...
if (isset($_POST['createCredit'])) {
    ...
    echo json_encode(['status' => 200, 'id' => $db->lastInsertId(), 'message' => 'Crédito creado']);
    exit;
}
if (isset($_POST['addCreditPayment'])) {
    ...
    $ref      = $_POST['reference'] ?? '';
    $idAdmin  = pos_int('id_admin');
    if (!$idCredit) { pos_fail(400, 'Crédito inválido'); }
    if ($amount <= 0) { pos_fail(400, 'El monto del abono debe ser mayor a 0'); }

    // Comprobante fuera de la transacción (I/O de filesystem)
    $filePath = null; $fileWarning = null;
    try {
        $filePath = posStoreSalePaymentFile($_FILES['proof'] ?? null);
    } catch (Exception $e) {
    ...
    try {
        ...
        if ($e->getMessage() === 'CREDIT_NOT_FOUND') { pos_fail(404, 'Crédito no encontrado'); }
        throw $e;
    }
    ...
    try {
        $id = pos_transaction($db, function(PDO $db) use ($idAdmin, $idOffice, $notes, $items) {
            ...
            foreach ($items as $item) {
                ...
                // Descontar inventario primero; si no afectó filas, no había stock suficiente
        ...
        if (strpos($msg, 'NO_STOCK:') === 0) {
            pos_fail(409, 'Stock insuficiente para el producto #' . substr($msg, 9) . '. Consignación cancelada.');
        }
        if ($msg === 'ITEM_INVALID') { pos_fail(400, 'Producto o cantidad inválidos'); }
        throw $e;
    }

    pos_ok(['id' => $id], 'Consignación creada');
}
...
if (isset($_POST['liquidateConsignment'])) {
    ...
    $idOffice = pos_int('id_office');
    $ref      = $_POST['reference'] ?? '';
    if (!$id) { pos_fail(400, 'Consignación inválida'); }
    ...
    try {
        pos_transaction($db, function(PDO $db) use ($id, $idOffice, $items, $filePath, $ref) {
            // Bloquear y verificar estado: evita liquidar dos veces (doble devolución de stock)
        ...
        if ($e->getMessage() === 'NOT_FOUND') { pos_fail(404, 'Consignación no encontrada'); }
        if ($e->getMessage() === 'ALREADY')   { pos_fail(409, 'Esta consignación ya fue liquidada'); }
        throw $e;
    }

    pos_ok(['file_consignment' => $filePath, 'file_warning' => $fileWarning], 'Liquidación completada');
}
