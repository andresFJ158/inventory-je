		...
		if(isset($_POST["scode_admin"])){
			...
			if($admin->status == 200){
				...
				<script>
					...
					console.error("Error en securityCode:", '.json_encode($admin).');
	...
	public function updateAdmin(){
		...
		if(isset($_POST["id_admin"])){
			...
			if($admin->status == 200){
				...
				// Preparar configuración de ChatGPT
				$chatgptConfig = array();
				if(isset($_POST["chatgpt_token"]) && !empty($_POST["chatgpt_token"])){
					$chatgptConfig["token"] = $_POST["chatgpt_token"];
					$chatgptConfig["org"] = isset($_POST["chatgpt_org"]) ? $_POST["chatgpt_org"] : "";
					$chatgptJson = json_encode($chatgptConfig);
				}else{
					// Mantener configuración existente si no se proporciona nueva
					$urlChatGPT = "admins?linkTo=id_admin&equalTo=".base64_decode($_POST["id_admin"])."&select=chatgpt_admin";
					$adminChatGPT = CurlController::request($urlChatGPT, "GET", array());
					if($adminChatGPT->status == 200 && !empty($adminChatGPT->results[0]->chatgpt_admin)){
						$chatgptJson = $adminChatGPT->results[0]->chatgpt_admin;
					}else{
						$chatgptJson = json_encode(array("token" => "", "org" => ""));
					}
				}
				...
				if($admin->results[0]->rol_admin == "superadmin"){
					...
					$fields = "email_admin=".$_POST["email_admin"]."&password_admin=".$crypt."&title_admin=".$_POST["title_admin"]."&symbol_admin=".$_POST["symbol_admin"]."&font_admin=".urlencode($_POST["font_admin"])."&color_admin=".$_POST["color_admin"]."&back_admin=".$_POST["back_admin"]."&chatgpt_admin=".urlencode($chatgptJson);

				}else{

					$fields = "email_admin=".$_POST["email_admin"]."&password_admin=".$crypt."&chatgpt_admin=".urlencode($chatgptJson);
				}
				...
				if($updateAdmin->status == 200){
					...
					// Actualizar configuración de ChatGPT en sesión si se proporcionó
					if(isset($_POST["chatgpt_token"]) && !empty($_POST["chatgpt_token"])){
						$_SESSION["admin"]->chatgpt_admin = $chatgptJson;
					}
					...
					<script>
						...
						console.error("Error en updateAdmin (update):", '.json_encode($updateAdmin).');
				...
				<script>
					...
					fncMatPreloader("off");
					fncFormatInputs();
					console.error("Error en updateAdmin (get):", '.json_encode($admin).');
		...
		if(isset($_POST["resetPassword"])){
			...
			if($admin->status == 200){
				...
				if($updatePassword->status == 200){
					...
					if($sendEmail == "ok"){
						...
						echo '<script>
							...
							fncMatPreloader("off");
							fncNotie("error", "'.$sendEmail.'");
							console.error("Error al enviar email en resetPassword");
					...
					echo '<script>
							...
							fncFormatInputs();
							fncMatPreloader("off");
							fncToastr("error", "Error al actualizar la contraseña");
							console.error("Error en resetPassword (update):", '.json_encode($updatePassword).');
				...
				echo '<script>
						...
						console.error("Error en resetPassword (get):", '.json_encode($admin).');
