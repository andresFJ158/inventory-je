			...
			if(isset($_POST["id_module"])){
				...
				if($updateModule->status == 200){
					...
					<script>
						...
						fncMatPreloader("off");
						fncFormatInputs();
					    fncSweetAlert("error","Error al actualizar el módulo", "");
						console.error("Error en manageModule (update):", '.json_encode($updateModule).');
			...
			=============================================*/
				...
				if($createModule->status == 200){
					...
					<script>
						...
						fncMatPreloader("off");
						fncFormatInputs();
					    fncSweetAlert("error","Error al crear el módulo", "");
						console.error("Error en manageModule (create):", '.json_encode($createModule).');
