...
const filtered = computed(() => {
  ...
  return suppliers.value.filter(s =>
    ...
    (s.supplier_contact || '').includes(q) ||
    (s.ruc_supplier || '').includes(q)
  )
})
...
function openCreate() {
  editing.value = null
  form.value = { id_supplier: 0, supplier_name: '', supplier_contact: '', email_supplier: '', ruc_supplier: '', status_supplier: 1 }
...
async function saveSupplier() {
  ...
  if (d?.status === 200) {
    ...
    toast.add({ title: d?.results || 'Error al guardar', color: 'error' })
  }
  saving.value = false
}
...
async function deleteSupplier(s: any) {
  ...
  if (d?.status === 200) {
    ...
    toast.add({ title: d?.results || 'Error al desactivar proveedor', color: 'error' })
  }
}
  ...
  <div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
      <div>
        ...
        <p class="text-xs text-slate-500 mt-0.5">Empresas y personas que abastecen materias primas al laboratorio.</p>
      </div>
      <div class="flex flex-wrap gap-2 w-full sm:w-auto">
        <UInput v-model="search" icon="i-lucide-search" placeholder="Buscar proveedor..." size="sm" class="flex-1 sm:w-52" />
    ...
    <div class="grid grid-cols-3 gap-3">
      ...
      <div class="bg-white border border-slate-200 rounded-xl p-4 text-center">
        <p class="text-2xl font-black text-green-600">{{ suppliers.filter(s => s.status_supplier == 1).length }}</p>
        <p class="text-xs text-slate-500 mt-0.5">Activos</p>
      </div>
      <div class="bg-white border border-slate-200 rounded-xl p-4 text-center">
        <p class="text-2xl font-black text-slate-400">{{ suppliers.filter(s => s.status_supplier != 1).length }}</p>
        <p class="text-xs text-slate-500 mt-0.5">Inactivos</p>
      </div>
    </div>
    ...
    <div v-else-if="filtered.length === 0" class="text-center py-16">
      <UIcon name="i-lucide-building-2" class="w-12 h-12 mx-auto text-slate-300 mb-3" />
      <p class="text-slate-500">No hay proveedores de laboratorio registrados</p>
    ...
    <USlideover v-model:open="slideOpen" :title="editing ? 'Editar Proveedor Lab' : 'Nuevo Proveedor Lab'">
      <template #body>
        <div class="space-y-4 p-1">
          <UFormField label="Nombre del Proveedor *">
            <UInput v-model="form.supplier_name" placeholder="Ej: Química Industrial SRL" class="w-full" />
          </UFormField>
