...
<template>
  <div class="max-w-3xl mx-auto py-8 px-4">
    ...
    <UCard class="backdrop-blur-sm bg-white/50 shadow-xl border border-slate-100">
      <div v-if="loading" class="py-12 flex justify-center">
        <UIcon name="i-lucide-loader-2" class="w-8 h-8 animate-spin text-primary" />
      </div>
      
      <div v-else class="space-y-8">
        <div class="bg-slate-50 border border-slate-200 rounded-2xl p-8 text-center shadow-inner">
          <h3 class="text-xs font-bold text-slate-400 mb-6 uppercase tracking-widest">Código QR Actual</h3>
          
          <div v-if="currentQR" class="flex flex-col items-center">
            <div class="p-3 bg-white rounded-2xl shadow-md border-2 border-slate-100 inline-block transform transition-transform hover:scale-105 duration-300">
              <img :src="getImageUrl(currentQR)" alt="QR Actual" class="max-w-[250px] sm:max-w-[300px] rounded-xl" />
            </div>
            <p class="text-sm font-medium text-emerald-600 mt-5 flex items-center gap-1.5">
          ...
          <div v-else class="py-8">
            ...
            <p class="text-slate-600 font-medium text-lg">No hay ningún código QR configurado</p>
        ...
        <div class="border-t border-slate-100 pt-8">
          ...
          <div class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1 w-full bg-white border border-slate-200 rounded-xl p-2 transition-all focus-within:border-primary focus-within:ring-1 focus-within:ring-primary">
              <label class="sr-only">Seleccionar nueva imagen</label>
              <input 
                ref="fileInput" 
                type="file" 
                accept="image/jpeg, image/png, image/webp" 
                class="block w-full text-sm text-slate-500 file:cursor-pointer file:mr-4 file:py-2.5 file:px-5 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100 transition-colors"
              />
            </div>
            <UButton 
              size="xl" 
