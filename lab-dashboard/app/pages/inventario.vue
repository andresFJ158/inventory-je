        ...
        <table v-else class="w-full text-left text-sm text-slate-650 dark:text-slate-350">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50/55 dark:hover:bg-slate-800/10 transition-colors duration-150">
              <td class="px-6 py-4">
                <div class="font-bold text-slate-800 dark:text-white uppercase tracking-wide flex items-center gap-2">
                  {{ item.name }}
                  <span v-if="item.is_insumo === 1 || item.is_insumo === '1'" class="px-2 py-0.5 rounded text-[10px] bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-200">Insumo</span>
                  <span v-else class="px-2 py-0.5 rounded text-[10px] bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-200">Materia Prima</span>
                </div>
              </td>
