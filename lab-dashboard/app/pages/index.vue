  ...
  <div class="space-y-6">
    ...
    <template v-else-if="isCashier">
      ...
      <UCard>
        ...
        <UTable v-else :rows="todaysOrders.slice(0, 5)" :columns="[{key: 'transaction_order', label: 'ID Transacción'}, {key: 'client', label: 'Cliente'}, {key: 'total_order', label: 'Total'}]">
          <template #client-data="{ row }">{{ decodeURIComponent(row.name_client).replace(/\+/g, ' ') }}</template>
          <template #total_order-data="{ row }"><span class="font-bold text-green-600">{{ formatCurrency(parseFloat(row.total_order)) }}</span></template>
        </UTable>
      </UCard>
    </template>
    ...
    <template v-else-if="isSuperAdmin">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <UCard>
          ...
          <UTable v-else :rows="openCashRegisters" :columns="[{key: 'title_office', label: 'Sucursal'}, {key: 'opening_balance_cash', label: 'Saldo Inicial'}]">
            <template #title_office-data="{ row }"><span class="font-semibold">{{ decodeURIComponent(row.title_office).replace(/\+/g, ' ') }}</span></template>
            <template #opening_balance_cash-data="{ row }">{{ formatCurrency(parseFloat(row.opening_balance_cash)) }}</template>
          </UTable>
        </UCard>
        ...
        <UCard>
          ...
          <UTable v-else :rows="todaysOrders.slice(0, 5)" :columns="[{key: 'date_order', label: 'Fecha/Hora'}, {key: 'total_order', label: 'Monto'}]">
            <template #date_order-data="{ row }"><span class="text-xs text-slate-500">{{ row.date_order.split(' ')[1] }}</span></template>
            <template #total_order-data="{ row }"><span class="font-semibold text-green-600">{{ formatCurrency(parseFloat(row.total_order)) }}</span></template>
          </UTable>
        </UCard>
      </div>
    </template>
