...
const columns = [
  { key: 'date_created_request', label: 'Fecha' },
  { key: 'title_warehouse', label: 'Almacén' },
  { key: 'title_product', label: 'Producto' },
  { key: 'qty_request', label: 'Solicitado' },
  { key: 'qty_dispatched', label: 'Despachado' },
  { key: 'status_request', label: 'Estado' },
  { key: 'notes_request', label: 'Notas' }
]
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div class="lg:col-span-7">
        <UCard>
          ...
          <UTable v-else :columns="columns" :rows="myRequests" class="w-full">
            <template #title_warehouse-data="{ row }">
              {{ formatText(row.title_warehouse) }}
            </template>
            <template #title_product-data="{ row }">
              <span class="font-semibold">{{ formatText(row.title_product) }}</span>
            </template>
            <template #qty_request-data="{ row }">
              {{ row.qty_request }}
            </template>
            <template #qty_dispatched-data="{ row }">
              <span :class="row.qty_dispatched > 0 ? 'text-green-600 font-bold' : 'text-slate-500'">
                {{ row.qty_dispatched || '0' }}
              </span>
            </template>
            <template #status_request-data="{ row }">
              <UBadge :color="getStatusColor(row.status_request)" variant="soft">
                {{ getStatusLabel(row.status_request) }}
              </UBadge>
            </template>
            <template #notes_request-data="{ row }">
              <span class="text-sm text-slate-500" :title="row.notes_request">
                {{ row.notes_request ? (row.notes_request.length > 20 ? row.notes_request.substring(0,20)+'...' : row.notes_request) : '-' }}
              </span>
            </template>
          </UTable>
        </UCard>
