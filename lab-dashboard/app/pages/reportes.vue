...
// Data table config
const orderCols = [
  { key: 'transaction_order', label: 'Transacción' },
  { key: 'client', label: 'Cliente' },
  { key: 'date_order', label: 'Fecha' },
  { key: 'method_order', label: 'Método' },
  { key: 'subtotal_order', label: 'Subtotal' },
  { key: 'discount_order', label: 'Dscto' },
...
const salesCols = [
  ...
  { key: 'qty_sale', label: 'Cant' },
  { key: 'price_sale', label: 'Precio' },
  { key: 'tax_sale', label: 'IVA%' },
  { key: 'discount_sale', label: 'Dscto%' },
  { key: 'subtotal_sale', label: 'Subtotal' }
]
</script>
...
<template>
  <div class="h-full flex flex-col p-6 space-y-6 overflow-y-auto w-full">
    ...
    <UTabs :items="items" class="w-full mt-4">
      <template #ordenes>
        <UCard class="mt-4">
           <UTable :rows="orders" :columns="orderCols" :loading="loading">
             <template #client-data="{ row }">
               {{ decodeStr(row.name_client) }} {{ decodeStr(row.surname_client) }}
             </template>
             <template #subtotal_order-data="{ row }">{{ formatCurrency(parseFloat(row.subtotal_order)) }}</template>
             <template #discount_order-data="{ row }">-{{ formatCurrency(parseFloat(row.discount_order)) }}</template>
             <template #total_order-data="{ row }">
               <span class="font-bold text-green-600">{{ formatCurrency(parseFloat(row.total_order)) }}</span>
             </template>
             <template #status_order-data="{ row }">
               <UBadge :color="row.status_order === 'Completada' ? 'green' : 'amber'" variant="subtle">
                 {{ row.status_order }}
               </UBadge>
             </template>
             <template #actions-data="{ row }">
               <UButton color="gray" variant="ghost" icon="i-lucide-file-text" size="sm" @click="viewPdf(row.id_order)">PDF</UButton>
             </template>
           </UTable>
        </UCard>
      </template>
      ...
      <template #ventas>
        <UCard class="mt-4">
           <UTable :rows="sales" :columns="salesCols" :loading="loading">
             <template #title_product-data="{ row }">
               <span class="font-medium">{{ decodeStr(row.title_product) }}</span>
             </template>
             <template #price_sale-data="{ row }">{{ formatCurrency(parseFloat(row.price_sale)) }}</template>
             <template #tax_sale-data="{ row }">{{ row.tax_sale }}%</template>
             <template #discount_sale-data="{ row }">{{ row.discount_sale }}%</template>
             <template #subtotal_sale-data="{ row }">
               <span class="font-bold">{{ formatCurrency(parseFloat(row.subtotal_sale)) }}</span>
             </template>
           </UTable>
        </UCard>
      </template>
