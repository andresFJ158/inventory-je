...
import { K as useAuthStore, ab as useToast, g as _sfc_main$h, h as _sfc_main$6, i as _sfc_main$c, j as _sfc_main$f } from './server.mjs';
import { _ as _sfc_main$2 } from './Badge-BLusyd6V.mjs';
import { _ as _sfc_main$3 } from './Slideover-CcB_tEFt.mjs';
import { _ as _sfc_main$4 } from './FormField-IGNQl_uA.mjs';
import { _ as _sfc_main$5 } from './Modal-DVs2bKsP.mjs';
import { defineComponent, ref, computed, watch, resolveComponent, mergeProps, withCtx, createVNode, toDisplayString, createTextVNode, withDirectives, vModelSelect, openBlock, createBlock, Fragment, renderList, createCommentVNode, useSSRContext } from 'vue';
      ...
      _push(ssrRenderComponent(_component_USlideover, {
        ...
        body: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (formModel.value.rol_admin !== "despachador") {
            ...
            if (formModel.value.rol_admin === "despachador") {
              _push2(ssrRenderComponent(_component_UFormField, { label: "Almacén Asignado" }, {
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`<select class="block w-full text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md px-2 py-1.5 focus:outline-none focus:border-indigo-500"${_scopeId2}><option value=""${ssrIncludeBooleanAttr(Array.isArray(formModel.value.id_warehouse_admin) ? ssrLooseContain(formModel.value.id_warehouse_admin, "") : ssrLooseEqual(formModel.value.id_warehouse_admin, "")) ? " selected" : ""}${_scopeId2}>Ninguno</option><!--[-->`);
                    ssrRenderList(warehouses.value, (w) => {
                      _push3(`<option${ssrRenderAttr("value", String(w.id_warehouse))}${ssrIncludeBooleanAttr(Array.isArray(formModel.value.id_warehouse_admin) ? ssrLooseContain(formModel.value.id_warehouse_admin, String(w.id_warehouse)) : ssrLooseEqual(formModel.value.id_warehouse_admin, String(w.id_warehouse))) ? " selected" : ""}${_scopeId2}>${ssrInterpolate(decodeURIComponent(w.title_warehouse || "").replace(/\+/g, " "))}</option>`);
                    });
                    _push3(`<!--]--></select>`);
                  } else {
                    return [
                      withDirectives(createVNode("select", {
                        "onUpdate:modelValue": ($event) => formModel.value.id_warehouse_admin = $event,
                        class: "block w-full text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md px-2 py-1.5 focus:outline-none focus:border-indigo-500"
                      }, [
                        createVNode("option", { value: "" }, "Ninguno"),
                        (openBlock(true), createBlock(Fragment, null, renderList(warehouses.value, (w) => {
                          return openBlock(), createBlock("option", {
                            key: w.id_warehouse,
                            value: String(w.id_warehouse)
                          }, toDisplayString(decodeURIComponent(w.title_warehouse || "").replace(/\+/g, " ")), 9, ["value"]);
                        }), 128))
                      ], 8, ["onUpdate:modelValue"]), [
                        [vModelSelect, formModel.value.id_warehouse_admin]
                      ])
                    ];
                  }
                }),
            ...
            return [
              createVNode("div", { class: "space-y-4 p-1" }, [
                ...
                formModel.value.rol_admin !== "despachador" ? (openBlock(), createBlock(_component_UFormField, {
                  key: 0,
...
//# sourceMappingURL=admins-DGwDMKLz.mjs.map
