import { H as useAuthStore, j as _sfc_main$6, g as _sfc_main$c, h as _sfc_main$h, i as _sfc_main$f, L as useComponentProps, a1 as useLocale, G as useAppConfig, W as useForwardProps, E as tv, U as useForwardExpose, f as Primitive, l as createContext } from './server.mjs';
...
import { _ as _sfc_main$5 } from './Slideover-DZ0IYow3.mjs';
import { _ as _sfc_main$7 } from './Switch-DkNSnXzc.mjs';
import { _ as _sfc_main$8 } from './Select-Di6-gJCC.mjs';
import { _ as _sfc_main$9 } from './Textarea-KHOF13zM.mjs';
import { _ as __nuxt_component_6 } from './OrderReceiptModal-B0P38A7N.mjs';
import { _ as _sfc_main$a } from './Modal-CoprpFuw.mjs';
import { _ as _sfc_main$b } from './Card-DphZz1jr.mjs';
        ...
        if (data.status === 200) {
          ...
          for (const col of cols) {
            ...
            if (col.type_column === "boolean") {
              ...
              const options = col.matrix_column ? col.matrix_column.split(",") : [];
              model[colName] = val !== void 0 && val !== "" ? String(val) : options.length > 0 ? options[0] : void 0;
              selectOptions.value[colName] = options;
            } else {
    ...
    async function loadRelationOptions(matrixTable) {
      ...
      try {
        ...
        if (data.status === 200 && data.results) {
          selectOptions.value[matrixTable] = data.results.filter((r) => {
            ...
            if (loading.value) {
              ...
              _push2(ssrRenderComponent(_component_UCard, { class: "bg-blue-50 dark:bg-blue-900/20 ring-blue-500/20" }, {
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`<div class="text-xs font-semibold text-blue-600 mb-1"${_scopeId2}>Monto Inicial</div><div class="text-xl font-bold"${_scopeId2}>${ssrInterpolate(formatCurrency(cashData.value.start_cash))}</div>`);
                  } else {
                    return [
                      createVNode("div", { class: "text-xs font-semibold text-blue-600 mb-1" }, "Monto Inicial"),
                      createVNode("div", { class: "text-xl font-bold" }, toDisplayString(formatCurrency(cashData.value.start_cash)), 1)
                    ];
                  }
                }),
                _: 1
              ...
              _push2(ssrRenderComponent(_component_UCard, { class: "bg-indigo-50 dark:bg-indigo-900/20 ring-indigo-500/20" }, {
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`<div class="text-xs font-semibold text-indigo-600 mb-1"${_scopeId2}>Total en Caja</div><div class="text-xl font-bold"${_scopeId2}>${ssrInterpolate(formatCurrency(Number(cashData.value.start_cash) + Number(cashData.value.money_cash) - Number(cashData.value.bills_cash)))}</div>`);
                  } else {
                    return [
                      createVNode("div", { class: "text-xs font-semibold text-indigo-600 mb-1" }, "Total en Caja"),
                      createVNode("div", { class: "text-xl font-bold" }, toDisplayString(formatCurrency(Number(cashData.value.start_cash) + Number(cashData.value.money_cash) - Number(cashData.value.bills_cash))), 1)
                    ];
                  }
                }),
                _: 1
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div><div class="table-compact-card bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">`);
      if (loading.value) {
        ...
        _push(`<div class="table-scroll"><table class="w-full text-left border-collapse text-sm text-slate-700 dark:text-slate-300"><thead><tr class="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800"><th class="table-index font-semibold text-xs uppercase">#</th><!--[-->`);
        ssrRenderList(visibleColumns.value, (col) => {
          _push(`<th class="font-semibold text-xs uppercase">${ssrInterpolate(col.alias_column || col.title_column)}</th>`);
        });
        _push(`<!--]-->`);
        if (hasRowActions.value) {
          _push(`<th class="table-actions text-right font-semibold text-xs uppercase">Acciones</th>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</tr></thead><tbody><!--[-->`);
        ssrRenderList(paginatedRows.value, (row, idx) => {
          _push(`<tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"><td class="table-index text-[11px] text-slate-400 dark:text-slate-500 font-mono">${ssrInterpolate((page.value - 1) * itemsPerPage + idx + 1)}</td><!--[-->`);
          ssrRenderList(visibleColumns.value, (col) => {
            _push(`<td>`);
            if (col.type_column === "image") {
              ...
              _push(`<span class="table-cell-code font-semibold text-teal-600 dark:text-teal-400"> Bs. ${ssrInterpolate(parseFloat(row[col.title_column] || 0).toFixed(2))}</span>`);
            } else if (col.type_column === "relations") {
              _push(`<span>`);
              _push(ssrRenderComponent(_component_UBadge, {
                color: "indigo",
              ...
              _push(ssrRenderComponent(_component_UBadge, {
                ...
                variant: "solid",
                size: "xs",
                class: "capitalize max-w-40 truncate"
              }, {
                default: withCtx((_, _push2, _parent2, _scopeId) => {
                  if (_push2) {
              ...
              _push(`<span class="${ssrRenderClass(isCompactCodeColumn(col.title_column) ? "table-cell-code text-xs text-slate-600 dark:text-slate-300" : "table-cell-text text-slate-700 dark:text-slate-300")}"${ssrRenderAttr("title", cellTitle(row[col.title_column]))}>${ssrInterpolate(decodeCell(row[col.title_column]))}</span>`);
            }
            _push(`</td>`);
          });
          _push(`<!--]-->`);
          if (hasRowActions.value) {
            _push(`<td class="table-actions text-right"><div class="flex items-center justify-end gap-1">`);
...
//# sourceMappingURL=DynamicTable-9zbkmh2W.mjs.map
