import { K as useAuthStore, g as _sfc_main$h, i as _sfc_main$c, h as _sfc_main$6 } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    async function deleteMaterial(item) {
      if (!confirm(`¿Eliminar la materia prima "${item.name}"?
...
Solo se puede eliminar si no tiene stock, no está en recetas ni en historial de producción.`)) return;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(` Catálogo de Materia Prima </h1><p class="text-slate-500 dark:text-slate-400 text-sm mt-1"> Gestión y registro de insumos base para formulaciones de laboratorio. </p></div>`);
      if (unref(auth).role === "lab_admin") {
        _push(ssrRenderComponent(_component_UButton, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(` Agregar Materia Prima `);
            } else {
              return [
                createTextVNode(" Agregar Materia Prima ")
              ];
            }
          }),
          _: 1
      ...
      if (unref(loading)) {
        ...
        _push(` Cargando catálogo desde la Base de Datos... </div>`);
      } else {
        ...
        if (unref(filteredItems).length === 0) {
          _push(`<tr><td colspan="7" class="px-6 py-8 text-center text-slate-400 text-sm">No se encontraron materias primas con ese criterio.</td></tr>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--[-->`);
        ssrRenderList(unref(filteredItems), (item, i) => {
          ...
          _push(`</td><td class="px-6 py-4 font-bold font-mono text-slate-800 dark:text-slate-100">${ssrInterpolate(item.stock.toFixed(2))}</td><td class="px-6 py-4"><span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold text-xs">${ssrInterpolate(item.unit)}</span></td><td class="px-6 py-4 text-xs text-slate-500 dark:text-slate-400 max-w-xs truncate">${ssrInterpolate(item.desc)}</td>`);
          if (unref(auth).role === "lab_admin") {
            ...
            _push(ssrRenderComponent(_component_UButton, {
              ...
              variant: "subtle",
              size: "xs",
              onClick: ($event) => deleteMaterial(item)
            }, null, _parent));
            _push(`</td>`);
          } else {
            _push(`<!---->`);
      ...
      _push(ssrRenderComponent(_component_UModal, {
        ...
        content: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="w-full p-6 space-y-4 text-slate-900 dark:text-white"${_scopeId}><div class="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-3"${_scopeId}><h3 class="text-lg font-bold text-slate-800 dark:text-white tracking-wide"${_scopeId}>${ssrInterpolate(unref(modalTitle))}</h3>`);
            ...
            return [
              createVNode("div", { class: "w-full p-6 space-y-4 text-slate-900 dark:text-white" }, [
                ...
                createVNode("form", {
                  class: "space-y-4",
                  onSubmit: withModifiers(saveMaterial, ["prevent"])
                }, [
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/materiales.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=materiales-DZqQH0li.mjs.map
