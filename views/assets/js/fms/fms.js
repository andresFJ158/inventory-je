		...
		$.ajax({
			...
			success: function(response){
				...
				try {
					...
					fncMatPreloader("off");
					fncToastr("error", "Error en la respuesta del servidor");
					return;
				}
				...
				if(jsonResponse.status == 200){
					...
					$(".columnName"+i).find("input").attr("idFile",jsonResponse.id_file);
					$(".columnName"+i).removeClass("columnName"+i);

					$(".progressList"+i).html(`<a href="${jsonResponse.link}" target="_blank">

						${jsonResponse.reduce_link}
					...
					$(".columnAction"+i).html(`<svg class="bi bi-copy copyLink" copy="${jsonResponse.link}" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" style="cursor:pointer">
						  <path fill-rule="evenodd" d="M4 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2zm2-1a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1zM2 5a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-1h1v1a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h1v1z"/>
						</svg>
					  <i class="bi bi-trash ps-2 btn deleteFile" idFile="${jsonResponse.id_file}" idFolder="${folder.split("_")[0]}" mode="list"></i>`);
					...
					$(".gridName"+i).find("input").attr("idFile",jsonResponse.id_file);
					$(".gridName"+i).removeClass("gridName"+i);

					$(".progressGrid"+i).html(`<a href="${jsonResponse.link}" target="_blank">
					...
					$(".gridAction"+i).html(`<svg class="bi bi-copy copyLink" copy="${jsonResponse.link}" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" style="cursor:pointer">
						  <path fill-rule="evenodd" d="M4 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2zm2-1a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1zM2 5a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-1h1v1a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h1v1z"/>
						</svg>
					  <i class="bi bi-trash ps-2 btn deleteFile" idFile="${jsonResponse.id_file}" idFolder="${folder.split("_")[0]}" mode="grid"></i>`);
						...
						$.ajax({
							...
							success: function(response){
								...
								if(response == 200){
									...
									fncMatPreloader("off");
									fncSweetAlert("error", "Error al actualizar el servidor", "");
									console.error("Error en updateServer response:", response);
								}

							},
							error: function(jqXHR, textStatus, errorThrown){
								fncMatPreloader("off");
								fncSweetAlert("error", "Error de red al actualizar el servidor", "");
								console.error("Error de red en updateServer:", textStatus, errorThrown);
							}
					...
					fncMatPreloader("off");
					fncToastr("error", jsonResponse.error || "Error desconocido en el servidor");
			...
			$.ajax({
				...
				success: function(response){
					...
					if(response == 200){
						...
						fncMatPreloader("off");
						fncSweetAlert("error", "Error al eliminar el archivo", "");
						console.error("Error en deleteFile response:", response);
					}

				},
				error: function(jqXHR, textStatus, errorThrown){
					fncMatPreloader("off");
					fncSweetAlert("error", "Error de red al eliminar el archivo", "");
					console.error("Error de red en deleteFile:", textStatus, errorThrown);
				}
	...
	$.ajax({
		...
		success: function(response){
			...
			if(response == 200){
				...
				fncToastr("error", "Error al cambiar el nombre del archivo");
				console.error("Error en updateName response:", response);
			}

		},
		error: function(jqXHR, textStatus, errorThrown){
			fncToastr("error", "Error de red al cambiar el nombre del archivo");
			console.error("Error de red en updateName:", textStatus, errorThrown);
		}
			...
			$.ajax({
				...
				success: function(response){
					...
					try {
						...
						console.error("Error parsing JSON response in loadFiles:", e);
						console.error("Raw response:", response);
						return;
					}
					$("#list table tbody").append(jsonResponse.htmlList);
					$("#grid").append(jsonResponse.htmlGrid);
